...
@Override
	...
	public List<Object[]> getTodaysEventCountDtlsForDashboard(
		...
		Session session = this.sessionFactory.getCurrentSession();
       String sql ="SELECT lift_customer_map_id,event_type,count(*) FROM rlms_event where (DATE(created_date)=CURDATE()) and rlms_event where lift_customer_map_id in("+str+") group by event_type,lift_customer_map_id order by lift_customer_map_id";
	    	SQLQuery query = session.createSQLQuery(sql);
		 	List<Object[]>EventCount = query.list();
			return EventCount;
